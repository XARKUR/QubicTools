  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          {date ? (
            `${t(`home.calculator.weekDays.${getDisplayDay(date)}`)} ${date.getHours().toString().padStart(2, '0')}:${date.getMinutes().toString().padStart(2, '0')}`
          ) : (
            t('home.calculator.startTime')
          )}
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-auto p-0" align="start">
        <div className="flex gap-2 p-2">
          <div className="flex flex-col gap-1">
            <div className="text-xs font-medium px-2 py-1.5">{t('home.calculator.weekDay')}</div>
            <ScrollArea className="h-[180px] w-[100px]">
              <div className="flex flex-col p-2 gap-1">
                {weekDays.map((day) => (
                  ...
                  >
                    {t(`home.calculator.weekDays.${day}`)}
                  </Button>
                ))}
              </div>
            </ScrollArea>
          ...
          <div className="flex flex-col gap-1">
            <div className="text-xs font-medium px-2 py-1.5">{t('home.calculator.hour')}</div>
          ...
          <div className="flex flex-col gap-1">
            <div className="text-xs font-medium px-2 py-1.5">{t('home.calculator.minute')}</div>
            <ScrollArea className="h-[180px] w-[60px]">
              <div className="flex flex-col p-2 gap-1">
                {minutes.map((minute) => (
                  <Button
