...
export default function BatchWallet() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (!count || count <= 0) {
      toast.error(t('batchWallet.toast.invalidCount'))
      return
    }

    const isSecure = forceStart || isOffline;
    if (!isSecure) {
      toast.error(t('batchWallet.toast.networkError'))
      return
    }
    ...
    try {
      ...
      worker.onerror = (error) => {
        ...
        toast.error(t('batchWallet.toast.generateError'))
      ...
      worker.onmessage = (event) => {
        ...
        if (publicId && seed) {
          ...
          if (completedRef.current === count) {
            setIsGenerating(false)
            worker.terminate()
            toast.success(t('batchWallet.toast.generateSuccess', { count }))
          }
        }
      }
    } catch (error) {
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <SidebarProvider>
        <AppSidebar />
        <SidebarInset>
          <PageHeader title={t('batchWallet.title')} />
              ...
              <div className="w-full min-w-0">
                <Card className="h-full">
                    ...
                    <div className="flex flex-col space-y-3">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <span className="text-xs whitespace-nowrap">{t('batchWallet.settings.forceStart.label')}</span>
                          <TooltipProvider>
                            <Tooltip>
                  ...
                  <CardContent className="p-6 pt-0 space-y-6">
                    ...
                    >
                      {isGenerating ? t('batchWallet.settings.generating', { progress }) : t('batchWallet.settings.start')}
                    </Button>
                    ...
                    <Card>
                      <CardContent className="p-4">
                        <div className="flex flex-col gap-4">
                          <div className="text-sm text-muted-foreground">
                            {t('batchWallet.mobile.description')}
                          </div>
                          <div className="flex flex-col sm:flex-row gap-4">
                            <div className="flex-1 flex items-center">
                              ...
                              >
                                ...
                                {t('batchWallet.store.googlePlay')}
                              </Button>
                              <TooltipProvider>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                            ...
                            <div className="flex justify-between items-start gap-4 flex-col sm:flex-row">
                              <div className="flex-1 min-w-0 space-y-2">
                                <div className="flex items-center">
                                  <span className="text-sm font-medium">{t('batchWallet.results.wallet')} #{index + 1}</span>
                                </div>
                                <div className="font-mono text-sm break-all">{wallet.publicId}</div>
                                <div className="text-xs text-muted-foreground">{t('batchWallet.results.scan')}</div>
                              </div>
